    ...
    if (legacy_overlay.length === 1) {
      ...
      try {
        let css_url = chrome.runtime.getURL(
          "libs/jquery-ui-1.12.1/jquery-ui.min.css"
        );
        ...
        console.log(
          "Не удалось прочитать файл libs/jquery-ui-1.12.1/jquery-ui.min.css"
        );
        return;
      }
...
function insertMonkeyIntoUI() {
  ...
  if (siem_title === "MaxPatrol 10") {
    ...
    navbaritem.append(
      `<img class="monkeydropbtn" width="32" height="32" src="${icondataurl}" alt="" />`
    );
    $(".monkeydropbtn")
      .delay(100)
      .fadeTo(100, 0.5)
...
function makeSideBarGreatAgain() {
  ...
  if (sidebar.length == 0) {
    ...
    if (sidebar.length == 0) {
      ...
      // ah! what the hell. dont know what im doing
      if (sidebar.length == 0) {
        sroot = $("legacy-events-page")[0].shadowRoot;
        sidebar = $(sroot).find(".mc-sidebar_right");
      }
    }
  }
  icons = sidebar.find(".pt-icons").first();
  icons.before(
    `<img class="blinkAndRemove" width="16" height="16" src="${icon16dateurl}" alt="" />`
  );
...
let observer = new MutationObserver(async (mutations) => {
  for (let mutation of mutations) {
    if (
      ...
      mutation.target.parentNode.nodeName === "EVENTS-FILTER-POPOVER"
    ) {
      // ━━━━━★. *･｡ﾟ✧⁺
      if (fields.length == 0) {
        ...
        fields = fieldsinfo
          ...
          .map((y) => y["name"]); //не все поля подходят для фильтров
        fields.push("=");
        fields.push("!=");
        fields.push(">");
        fields.push("<");
      ...
      ta.on("keydown", function (event) {
        ...
        open: function () {
          ...
          $(".ui-autocomplete", ta.parent()).position({
            my: "left top",
            at: `left+${x} top+${y}`,
            of: ta,
          });
          ...
          console.log(items.length); // debug
    ...
    if (
      ...
      "options" in options &&
      "dont_show_desc_rules" in options.options &&
      options.options.dont_show_desc_rules === false
    ) {
      let correlation_name_node = $(
        mutation.target.parentNode.parentNode
      ).children("span.mc-link.ng-scope");
      $(mutation.target.parentNode.parentNode)
        .children("span.corr-desc")
        .remove();
      ...
      let [correlation_description, correlation_link_to_ptkb] =
        corr_name_info(msg);

      setTimeout(
        AddElementIfNotExist,
        0,
        correlation_name_node,
    ...
    //Добавляет описание и ссылку на правило в разделе "инциденты"
      ...
      let correlation_name_node = $(mutation.target.parentNode).children(
        "span.mc-link.ng-scope"
      );
      let correlation_name = correlation_name_node.text();
      let msg = await getCorrelationRuleInfoByName(correlation_name);

      let corr_desc_old = $(mutation.target.parentNode)
        .children("span.corr-desc")
        .text();

      if (
        !corr_desc_old ||
        (corr_desc_old && msg["description"] !== corr_desc_old)
      ) {
    ...
    for (let addedNode of mutation.addedNodes) {
      ...
      if (
        addedNode instanceof Node &&
        addedNode.className === "mc-dl-conditional ng-scope"
      ) {
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "uuid")
        ) {
          if (
            "options" in options &&
        ...
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML.endsWith(".hash"))
        ) {
          if (
            "options" in options &&
            "hashlinks" in options.options &&
            options.options.hashlinks.length > 0
          ) {
            ObjectHashAdd(addedNode, addedNode.children[0].innerHTML);
          } else {
            CommonFieldClick(
              addedNode,
              addedNode.children[0].innerHTML,
              GetVirusTotalLinkForHash
            );
          }
        }
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "external_link")
        ) {
          CommonFieldClick(addedNode, "external_link", GetExternalLink);
        }
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "task_id")
        ) {
          CommonFieldClick(addedNode, "task_id", GetTaskLink);
        }
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "id")
        ) {
        ...
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "correlation_name")
        ) {
          await fieldAliases(addedNode);
        }
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "src.ip")
        ) {
          await ipfieldChangeObserver(addedNode, "src.ip");
        }
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "dst.ip")
        ) {
          await ipfieldChangeObserver(addedNode, "dst.ip");
        }
        if (
          (addedNode.children.length =
            2 && addedNode.children[0].innerHTML === "object")
        ) {
...
function corr_name_info(msg) {
  ...
  let correlation_link_to_ptkb = $(
    '<i title="Открыть правило в Knowledge Base">'
  )
    .addClass("pt-icons")
...
async function getdata(
  ...
  $.ajax({
    ...
    error: function (msg) {
      $(`#output${outputelemsuffix}`).text(
        "Что-то пошло не так, дерева не будет, может быть в другой раз..."
      );
    },
  });
}
...
function ProcessHandler(addedNode) {
  ...
  let session_tree_icon = $(
    `<span title="Дерево процессов сессии...">🦍</span>`
  );
  ...
  ancestors_branch_icon.click(function () {
    ...
    element = $(
      'div[title="object"] + div span.pt-preserve-white-space',
      addedNode
    );
    newelem = $("<div>")
      .attr("id", "output")
      .attr("title", `Родители процесса ${commandline}`)
      .dialog({
        height: h - 100,
        width: w - 100,
  ...
  descendants_tree_icon.click(function () {
    ...
    newelem = $("<div>")
      .attr("id", "output")
      .attr("title", `Потомки процесса ${commandline}`)
      .dialog({
        height: h - 100,
        width: w - 100,
        close: function (event, ui) {
    ...
    if (msgid === "1" || msgid === "4688") {
      getdata(
        ...
        `uuid = '${uuid}'`,
        count,
        processTreeBranchReverse,
        "",
        ttimeto - 86400 - 600,
        ttimeto
      ); //TODO: со временем путаница и не удобно, надо распутаться
    }
    // Иначе ищем доступными средствами соответсвующее событие запуска процесса
    else {
      getdata(
...
function getFieldValueFromSidebar(fieldName) {
  ...
  if (legacy_events_page.length === 1) {
    let shadowRoot = legacy_events_page[0].shadowRoot;
    let fieldValue = $(
      `div[title=\"${fieldName}\"] + div > div > div:first`,
      shadowRoot
    )
      .text()
      .trim("↵");
  ...
  let fieldValue = $(
    ...
    .trim("↵");
  if (fieldValue == "") {
    fieldValue = $(`div[title=\"${fieldName}\"] + div > div > div:first`)
      .text()
      .trim("↵");
  }
  return fieldValue;
...
function getTimeValueFromSidebar() {
  ...
  if (legacy_events_page.length === 1) {
    ...
    let time = $(
      "mc-sidebar-opened > header > div.layout-row.flex > div > div",
      shadowRoot
    )
      .text()
      .trim("↵");
  ...
  let time = $(
    "body > section > div > div > events-page > div > section > mc-sidebar.mc-sidebar_wide.mc-sidebar_right.ng-scope.ng-isolate-scope > mc-sidebar-opened > header > div.layout-row.flex > div > div"
  )
    .text()
    .trim("↵");
  if (time.length === 0) {
    time = $("mc-sidebar-opened > header > div.layout-row.flex > div > div")
      .text()
      .trim("↵");
    if (time.length === 0) {
      let iframe = $("#legacyApplicationFrame");
...
function DstIPAdd(addedNode) {
  ...
  dst_ip_span = $(
    'div[title="dst.ip"] + div span.pt-preserve-white-space',
    addedNode
  );
  ...
  if ("options" in options && "iplinks" in options.options) {
    options.options.iplinks.forEach((e) => {
      ...
      AddExternalServiceLink(dst_ip_span, e.name, (ip) =>
        e.template.replace("${ip}", ip)
      );
    });
  }
}
...
function SrcIPAdd(addedNode) {
  ...
  src_ip_span = $(
    'div[title="src.ip"] + div span.pt-preserve-white-space',
    addedNode
  );
  ...
  if ("options" in options && "iplinks" in options.options) {
    options.options.iplinks.forEach((e) => {
      let link = e.template.replace("${ip}");
      AddExternalServiceLink(src_ip_span, e.name, (ip) =>
        e.template.replace("${ip}", ip)
      );
    });
  }
...
function ObjectHashAdd(addedNode, fieldname) {
  ...
  object_hash_span = $(
    `div[title=\"${fieldname}\"] + div span.pt-preserve-white-space`,
    addedNode
  );
...
function AddExternalServiceLink(src_ip_span, text, callback) {
  ...
  vtdiv.hover(
    function () {
      let ip_to_check = $(".pt-preserve-white-space", $(this).parent())
        .text()
        .replace(/[^.0-9]+/g, "");
...
async function GetExternalLink(external_link) {
  ...
  title = $(
    'div[title="event_src.title"] + div pdql-fast-filter span.pt-preserve-white-space'
  )
    .text()
    .trim("↵");
  if (title == "multiscanner") {
    ...
    sandboxUrl = apps.filter((x) => {
      return x.type === "PT.MS" && !x.endpoint.includes(":8443");
    })[0]["endpoint"];
...
function AddElementIfNotExist(
  value_node_span,
  descendants_tree_icon,
  classname
) {
  let legacy_events_page = $("legacy-events-page");
  if (legacy_events_page.length === 1) {
...
function AddDownloadNormalizedSubeventsIcon(addedNode) {
  ...
  download_all_subevents_icon = $(
    `<span title="Сохранить JSON всех исходных событий в файл">🖫</span>`
  );
  ...
  setTimeout(
    ...
    value_node_span,
    download_all_subevents_icon,
    ".downloadsubeventsnormalizedicon"
  );
  download_all_subevents_icon.click(function () {
    ...
    getdata(
      ...
      `uuid = '${uuid}'`,
      1,
      processCorrleationEventDownloadSubevents,
      "",
      ttimeto,
      ttimeto
    ); //TODO: со временем путаница и не удобно, надо распутаться
...
function AddDownloadNormalizedIcon(addedNode) {
  ...
  copy_normalized_icon = $(
    `<span title="Скопировать JSON события в буфер обмена">📋</span>`
  ); //
  ...
  download_normalized_icon = $(
    `<span title="Сохранить JSON события в файл">💾</span>`
  ); //
  ...
  download_normalized_icon.click(function () {
    ...
    getdata(
      ...
      `uuid = '${uuid}'`,
      1,
      processCorrleationEventDownload,
      "",
      ttimeto,
      ttimeto
    );
...
function AddGetShareableEventLinkIcon(addedNode) {
  ...
  link_icon = $(
    `<i class="pt-icons pt-icons-link_16" title="Ссылка на это событие"></i>`
  );
  ...
  link_icon.click(function () {
    ...
    $("<div>Ссылка в буфере обмена...</div>")
      .insertAfter(icon)
      .show()
      .delay(500)
      .fadeOut();
...
async function popup_event_handler() {
  ...
  try {
    ...
    fields.forEach((x) => {
      params[x.name] = $(
        `div[title=\"${x.name}\"] + div > div > div:first`,
        applicationNode
      )
        .text()
        .trim("↵");
    ...
    params["nad_src_ip"] = $(
      'details-endpoint[dir="src"] span[ng-if="::details[dir].ip"]',
      applicationNode
    )
      .first()
      .text();
    params["nad_dst_ip"] = $(
      'details-endpoint[dir="dst"] span[ng-if="::details[dir].ip"]',
      applicationNode
    )
      .first()
      .text();
    params["nad_src_port"] = $(
      'details-endpoint[dir="src"] span[ng-if="::details[dir].port"]',
      applicationNode
    )
      .eq(1)
      .text();
    params["nad_dst_port"] = $(
      'details-endpoint[dir="dst"] span[ng-if="::details[dir].port"]',
      applicationNode
    )
      .eq(1)
      .text();
    params["session_start"] = $(
      'div[row-title="Начало"]',
      applicationNode
    ).attr("row-value");
  }
...
async function applyFieldAliases(changedElement) {
  ...
  let id = $(`div[title=\"id\"] + div > div > div:first`, sidebar)
    .text()
    .trim("↵");
  ...
  let correlation_name = $(
    `div[title=\"correlation_name\"] + div > div > div:first`,
    sidebar
  )
    .text()
    .trim("↵");
...
GetOptionsFromStorage().then(() => {
  ...
  if (
    "options" in options &&
    "disable_agg_sort" in options.options &&
    options.options.disable_agg_sort == true
  ) {
